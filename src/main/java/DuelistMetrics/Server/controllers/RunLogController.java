...
import DuelistMetrics.Server.util.timing.Timing;
...
public class RunLogController {
    ...
    public static RunLogService getService() {
        return bundles;
    }
    ...
    public record RunDetailsID(String host, BigDecimal localTime) {
    }
    ...
    public static ResponseEntity<?> getRunDetails(@RequestBody RunDetailsID runDetailsInfo) {
        ...
        var canLookup = host != null && !host.isEmpty() && localTime != null;
        if (canLookup) {
    ...
    private record DeckCardsHolder(String deckName, List<Integer> basicCards, List<Integer> poolCards,
                                   List<Integer> deckCards, List<Integer> relics, List<Integer> potions) {
    ...
    public static Collection<DisplayDeckV2> getDeckCompare() {
            ...
            if (allKilledList.size() > 0) {
                String[] splice = allKilledList.getFirst().split(",");
            ...
            for (var deckName : decks) {
                if (!deckName.equals("NotYugi") && !deckName.contains("Random") && !deckName.equals("Upgrade Deck")) {
                    ...
                    for (var split : splice) {
                        ...
                        out.append(!split.isEmpty() ? " " + split : split);
                    }
            ...
            displayDeckV2s.addFirst(new DisplayDeckV2("All", allDeck.getMostKilledBy(), allDeck.getRuns(), allDeck.getWins(),
