...
WHERE oc.name like 'theDuelist:%' OR oc.name in :relicIds
GROUP BY oc.name
...
WHERE pi.deck = :deck AND (oc.name like 'theDuelist:%' OR oc.name in :relicIds)
GROUP BY oc.name
...
@NamedNativeQuery(name = "getInfoRelicRelicListFromDeckV2Lookup", query = """
SELECT
    ir.relic_id AS uuid,
    ir.name AS name,
    ir.tier AS rarity,
...
FROM info_relic ir
WHERE ir.relic_id in :relicIds AND
      ir.info_info_bundle_id = (SELECT MAX(info_bundle_id) FROM mod_info_bundle WHERE is_duelist = 1)
GROUP BY ir.relic_id
""", resultSetMapping = "v2InfoRelicFullInfoDisplayObjectRelicDeckMapping")
@SqlResultSetMapping(
        name = "v2InfoRelicFullInfoDisplayObjectRelicDeckMapping",
        classes = @ConstructorResult(targetClass = FullInfoDisplayObject.class,columns = {
                @ColumnResult(name = "uuid", type = String.class),
