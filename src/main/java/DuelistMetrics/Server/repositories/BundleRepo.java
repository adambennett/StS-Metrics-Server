...
public interface BundleRepo extends JpaRepository<Bundle, Long> {
    ...
    @Query(value = "SELECT top_id, victory, sc.picked, sc.floor, starting_deck FROM bundle JOIN spire_card sc on bundle.top_id = sc.bundle_top_id WHERE starting_deck = :deck AND sc.floor < 51 AND ascension_level >= :ascensionHigherThan", nativeQuery = true)
    List<String> getBundlesForTierScores(String deck, int ascensionHigherThan);

    @Query(value = "SELECT top_id, victory, sc.picked, sc.floor, starting_deck FROM bundle JOIN spire_card sc on bundle.top_id = sc.bundle_top_id WHERE starting_deck = :deck AND sc.floor < 51 AND challenge_level >= :challengeHigherThan", nativeQuery = true)
    List<String> getBundlesForTierScores(int challengeHigherThan, String deck);

    @Query(value = "SELECT top_id, victory, sc.picked, sc.floor, starting_deck FROM bundle JOIN spire_card sc on bundle.top_id = sc.bundle_top_id WHERE starting_deck = :deck AND sc.floor < 51 AND ascension_level >= :ascensionHigherThan AND challenge_level >= :challengeHigherThan", nativeQuery = true)
