...
public interface TierScoreRepo extends JpaRepository<ScoredCard, ScoredCardKey> {
    ...
    @Query(value = "SELECT pool_name, card_id, card_name, overall_score, act0_score, act1_score, act2_score, act3_score FROM scored_card WHERE pool_name = :pool", nativeQuery = true)
